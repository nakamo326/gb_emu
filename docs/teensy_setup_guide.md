...
揃うはず) ことから、単純な初期化順序のバグよりも、電源/GND経由のノイズによる ST7789 の
RST ラインへの偶発的なグリッチ、または SAI1 の割り込み (`FIFO_REQUEST`, 数kHz 相当で発生) と
`display.rs` の DMA 制御 (特に生レジスタ操作の `finalize_continuous_transfer()`) との間の
低頻度なタイミング競合、のいずれかが濃厚と見ている。いずれも未確定。

#### 未検証・次の一手候補

- ディスプレイモジュールの VCC-GND 直近にバイパスコンデンサ (0.1µF セラミック + 数十µF 電解)
  を追加し、電源/GND 経由のノイズ耐性を上げて再現するか確認する
- ロジックアナライザ/オシロで ST7789 の RST ピンを監視し、黒画面になる瞬間に実際にパルスが
  入っていないか確認する
- `audio.rs` の `on_sai1_interrupt()` を NVIC 優先度を下げる、または DMA 転送中は割り込みを
  一時的にマスクするなど、`display.rs` の DMA 制御と時間的に排他にしてみる
