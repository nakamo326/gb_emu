...
#### 音のざらつき (ジャギジャギ感) の低減: エイリアシング対策 (2026-07-21)

テンポ正常化後も「原音と比べてざらついて聞こえる」という報告があった。原因は APU の
ダウンサンプリングが「サンプル時点の瞬時値の間引き」だったこと。矩形波/ノイズ
(最大 1MiHz 更新) の高調波がナイキスト (22.05kHz) で折り返し、非整数次の耳障りな
成分 (エイリアシング) として乗っていた。

対策: `core/src/apu.rs` の `emulate_cycle()` で毎 M-cycle ミキサー出力を積算し、
サンプル境界で平均を出力するボックスフィルタ方式に変更 (デシメーション前の帯域制限)。
`host/examples/audio_dump.rs` (WAV ダンプ + 簡易統計) による A/B 比較で、高域
エネルギー比 (一次差分/全体) が 0.0189 → 0.0126 (約 33% 減)、RMS はほぼ不変である
ことを確認した。CPU コストは mix() が毎 M-cycle 呼ばれる分わずかに増える
(Teensy 実機で数 % 程度の見込み。負荷はシリアルログの peak/avg で確認可能)。
