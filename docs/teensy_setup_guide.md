...
### 音声が途切れ途切れ・全体的に遅い/ピッチが低い (対策済み・実機検証待ち, 2026-07-21)
...
#### 原因分析と対策: オーディオマスター同期へ変更 (2026-07-21)

コード解析により、ログ/オーバーレイとは独立に、音声パスの設計自体に症状を説明できる
構造問題が 3 つあることを特定した:

1. **リングバッファが常時「ほぼ空」の平衡点で動作する**: SAI の実消費レートは
   MCLK 11,293,920 Hz ÷ bclk_div(8) ÷ 32bit ≈ 44,116.9 Hz で、APU の生成レート
   (DWT ペーシング下で実時間換算ちょうど 44,100 Hz) より 0.04% 速い。差はわずか
   毎秒 ~17 サンプルだが、ドロップ方式のバッファには不足分を取り戻す機構がないため、
   バッファ残量は単調に減って必ず空近傍に張り付く。この状態では `draw()` や割り込み等の
   数 ms のジッタが毎回そのまま無音ギャップとして出力される (= 途切れ途切れの正体)。
2. **締切超過時に遅延を取り戻さない**: フレーム締切超過時は `next_deadline` を現在時刻に
   リセットするだけで追い上げないため、一度発生した遅延が蓄積し、音楽が実時間から
   累積的に遅れていく (= 全体的に遅い、の正体。従来からの推測どおり)。
3. **LCD オフ期間の暴走**: LCD 無効中は `frame_ready` が来ずペーシングが一切効かないため
   エミュレーションが全速走行し、生成がバッファ容量 (23ms) を超えてサンプル大量ドロップ
   になる (画面遷移時の音の乱れ)。

対策 (`audio.rs` / `main.rs`):

- **`SaiAudio::push()` をバッファ満杯時ブロッキングに変更 (オーディオマスター同期)**。
  エミュレーション全体が DAC の実サンプルレートに正確にロックされ、バッファは常時
  ほぼ満杯 (≈23ms のヘッドルーム、リング初期状態もサイレンス満杯で開始) を維持する。
  クロックずれ・ジッタ起因のアンダーランが構造的に消え、LCD オフ中もペーシングが効く。
  SAI 停止時のハング防止として 1ms タイムアウトでペーシングを恒久無効化し従来の
  ドロップ動作へフォールバックする (`log::warn!` を出力)。
- **DWT 締切は名目フレーム周期より 5% 速いセーフティキャップに変更**。通常運転では
  発動せず、(1) オーディオ停止時の暴走防止、(2) アンダーラン発生後にバッファを
  再充填する際の追い上げ上限 (+5%、約 0.5 秒で全回復) を担う。遅延はもう蓄積しない。
- `record_work()` の負荷計測からはペーシング待ち時間 (`take_blocked_cycles()`) を
  除外したため、オーバーレイの負荷% は従来どおり実処理時間を示す。

副作用: 音声レイテンシがバッファ満杯ぶんの約 23ms (一定) になる。従来の「ほぼ 0ms だが
不安定」から「23ms 固定で安定」への交換であり、体感上の問題はないはず。

なお「1 秒に 1 回の数百 ms 停滞」(前述の注記) が render_overlay 無効化後も残存する場合、
23ms を超える停滞はその瞬間の音切れとしては残る (ただし遅延の蓄積はしない)。停滞が
残るかは実機のシリアルログ (`peak:`) で別途確認すること。

#### 真因判明: CGB ダブルスピードモードの未実装 (2026-07-21)

オーディオマスター同期の導入後も「音楽がゆっくり聞こえる」症状が残ったため再調査した
結果、「全体的に遅い」の真因は音声パスではなく **core の CGB ダブルスピード (KEY1) の
エミュレーション方法**だったことが判明した。

- 従来の core は `GameBoy::step()` で CPU・タイマー・PPU・APU をすべて 1:1 で刻み、
  ダブルスピードを「マシン全体が 2 倍速」として近似していた (host が `cycle_ns/2` で
  補償する前提)。実機ではダブルスピード中も PPU/APU クロックは等速のままで、
  2 倍になるのは CPU とタイマーだけである。
- Teensy はオーディオ (= APU レート) でペーシングするため、この近似の下では
  ダブルスピード中の CPU・タイマーが**実機の半分の速度**で動く。ZELDA_NAYRU は
  起動直後 (step 16623) に KEY1 切替することをヘッドレス実行 (`host/examples/
  speed_check.rs`) で確認した。タイマー割り込み駆動のサウンドドライバは
  ちょうど半分のテンポになり、「音楽がゆっくり」の症状と一致する。
- 対策: core の `step()` を変更し、ダブルスピード中は PPU/APU を 1 step おきに進める
  ようにした (CPU・タイマーは毎 step)。修正後、ダブルスピード中のフレーム間隔が
  17556 → 35112 step になり、CPU/タイマーに実機同等のサイクルが供給されることを
  speed_check で確認済み。blargg cpu_instrs・全ユニットテストも通過。
- この修正は host にも正しく効く (`cycle_ns/2` と組み合わせて PPU 59.7fps /
  APU 44100Hz / CPU 2 倍が成立する)。
